...
import torch
from train.td3_dann_halfcheetah import DANNActor  # 确保路径正确
...
def make_env():
    return DummyVecEnv([lambda: gym.make("HalfCheetah-v5")])

def load_custom_model(model_path):
    # 构建和训练时一致的环境
    env = make_env()

    # 初始化一个 TD3 模型（带 dummy actor，之后替换）
    model = TD3("MlpPolicy", env)
    
    # 替换 actor 和 target
    obs_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    actor = DANNActor(obs_dim, action_dim)
    actor_target = DANNActor(obs_dim, action_dim)

    model.policy.actor = actor
    model.policy.actor_target = actor_target
...
if __name__ == "__main__":
    ...
    model_path = "./runs/td3_dann_2025-04-22_18-40-58/td3_halfcheetah_dann.zip"
